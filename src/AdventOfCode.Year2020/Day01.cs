namespace AdventOfCode.Year2020
{
    ...
    public sealed class Day01 : BaseDay
    {
        ...
        public override string Solve_1() => _input.SelectMany(_ => _input, (x, y) => new {x, y})
            .Where(t => t.x + t.y == 2020)
            .Select(t => t.x * t.y).First().ToString();

        public override string Solve_2() => Part2_FastLinq();

        internal string Part2_FastLinq() => _input.Where(input1 =>
                _input.Find(input2 => _input.Contains(2020 - input1 - input2)) != default)
            .Aggregate(1, (o, c) => o * c)
            .ToString();

        internal string Part2_SlowLinq() => _input.SelectMany(x => _input, (x, y) => new {x, y})
            .SelectMany(t => _input, (t, z) => new {t, z})
            .Where(t => t.t.x + t.t.y + t.z == 2020)
            .Select(t => t.t.x * t.t.y * t.z).First().ToString();
    }
}
